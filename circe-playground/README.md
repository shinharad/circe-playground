# circe-playground

## Part 1 - Introduction to working with #JSON in #Scala
- https://www.youtube.com/watch?v=712WE9Ou7BE

## Part 2 - #JSON Codecs
- https://www.youtube.com/watch?v=OJhAptqtwYo

## Part 3.1 - Integrations & Extras (refined, lightbend config, fs2, http4s and Scala.js)
- https://www.youtube.com/watch?v=0yMoYU-rodw

## Part 3.2 - Integrations & Extras (refined, lightbend config, fs2, http4s and Scala.js)
- https://www.youtube.com/watch?v=L_u7uBHpmAU&t=6s


## other

GitHub API
- https://api.github.com/
- https://api.github.com/users/shinharad/repos